...
def geoip_lookup(query=None):
    """Look up geolocation data for an IP address via ip-api.com (free, no key)."""
    if query:
...
def display_geoip(data):
    ...
    fields = [
        ...
        ("Country Code", "countryCode"), ("Region", "regionName"),
        ("Region Code", "region"), ("City", "city"),
        ("ZIP / Postal", "zip"), ("Latitude", "lat"),
        ("Longitude", "lon"), ("Timezone", "timezone"),
        ("ISP", "isp"), ("Organization", "org"),
        ("AS Number", "as"), ("AS Name", "asname"),
    ]
